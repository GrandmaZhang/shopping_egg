...
class GoodsService extends Service {
  async getRandomGoods(number) {
    const { app } = this;
    const randomGoods = await app.mysql.query('select * from goods where isDeleted = ? order by rand() limit 0, ?', [ 0, number ]);
    return randomGoods;
  }
  async getBanner() {
    const { app } = this;
    const banner = await app.mysql.select('goods', {
      where: { isBanner: 1, isDeleted: 0 },
    });
    return banner;
  }
  ...
  async addGoodsCart(goodsId, number, userId) {
    ...
    if (item.length) {
      ...
      await app.mysql.insert('shopping_cart', { goodsId, goodsNumber: number, createTime: new Date(), userId });
    }
    return true;
  }
  async getSearchGoods(keyword) {
    const { app } = this;
    const searchResult = await app.mysql.query('select * from goods where isDeleted = 0 and name like ?', [ '%' + keyword + '%' ]);
    return searchResult;
  }
  async getCartItem(userId) {
    const { app } = this;
    const result = await app.mysql.query('select * from shopping_cart inner join goods on shopping_cart.goodsId = goods.id and shopping_cart.userId = ?', [ userId ]);
    return result;
  }
